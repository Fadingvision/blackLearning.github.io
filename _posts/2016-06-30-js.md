...
#### jQuery的存在只是为了解决丑陋的API,以及浏览器兼容的问题,在es6结合babel以及浏览器的进步的今天，加上mvvm开发模式的盛行，直接操作dom或许不是一种很好的方式，所以我们或许不再需要Jquery,下面介绍了一些Jquery的api的native替代方式。
